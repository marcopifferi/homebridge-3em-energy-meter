...
function EnergyMeter(log, config) {
  ...
  this.use_shelly = config["use_shelly"] || false;
  ...
  this.service
    .getCharacteristic(this._EvePowerConsumption)
    .on("get", this.getPowerConsumption.bind(this));
  this.service
    .addCharacteristic(this._EveTotalConsumption)
    .on("get", this.getTotalConsumption.bind(this));
  this.service
    .addCharacteristic(this._EveVoltage1)
    .on("get", this.getVoltage1.bind(this));
  this.service
    .addCharacteristic(this._EveAmpere1)
    .on("get", this.getAmpere1.bind(this));
...
EnergyMeter.prototype.updateState = function () {
  if (this.waiting_response) {
    this.log(
      "Please select a higher update_interval value. Http command may not finish!"
    );
    return;
  }
  ...
  this.last_value = new Promise((resolve, reject) => {
    ...
    request(ops, (error, res, body) => {
      ...
      if (error) {
        ...
        try {
          ...
          if (this.use_shelly) {
            this.powerConsumption = Math.abs(parseFloat(json.meters[0].power));
            ...
            if (this.use_em_mode == 0) {
              this.powerConsumption = Math.abs(
                parseFloat(json.emeters[0].power) +
                  parseFloat(json.emeters[1].power)
              );
              this.totalPowerConsumption = Math.abs(
                (parseFloat(json.emeters[0].total) +
                  parseFloat(json.emeters[1].total)) /
                  1000
              );
              this.voltage1 = Math.abs(
                (parseFloat(json.emeters[0].voltage) +
                  parseFloat(json.emeters[1].voltage)) /
                  2
              );
              ...
              if (this.use_em_mode == 1) {
                ...
                this.totalPowerConsumption = Math.abs(
                  parseFloat(json.emeters[0].total) / 1000
                );
                ...
                if (this.use_em_mode == 2) {
                  ...
                  this.totalPowerConsumption = Math.abs(
                    parseFloat(json.emeters[1].total) / 1000
                  );
                  this.voltage1 = Math.abs(parseFloat(json.emeters[1].voltage));
                  this.ampere1 = Math.abs(
                    this.powerConsumption / this.voltage1
                  );
                }
              }
            }
            ...
            this.powerConsumption = Math.abs(
              parseFloat(json.emeters[0].power) +
                parseFloat(json.emeters[1].power) +
                parseFloat(json.emeters[2].power)
            );
            this.totalPowerConsumption = Math.abs(
              (parseFloat(json.emeters[0].total) +
                parseFloat(json.emeters[1].total) +
                parseFloat(json.emeters[2].total)) /
                1000
            );
            this.voltage1 = Math.abs(
              (parseFloat(json.emeters[0].voltage) +
                parseFloat(json.emeters[1].voltage) +
                parseFloat(json.emeters[2].voltage)) /
                3
            );
            this.ampere1 = Math.abs(
          ...
          if (this.debug_log) {
            this.log(
              "Successful http response. [ voltage: " +
                this.voltage1.toFixed(0) +
                "V, current: " +
                this.ampere1.toFixed(1) +
                "A, consumption: " +
                this.powerConsumption.toFixed(0) +
                "W, total consumption: " +
                this.totalPowerConsumption.toFixed(2) +
                "kWh ]"
            );
          }
        } catch (parseErr) {
    ...
    (value_current, value_total, value_voltage1, value_ampere1) => {
      if (value_current != null) {
        this.service
          .getCharacteristic(this._EvePowerConsumption)
          .setValue(value_current, undefined, undefined);
        //FakeGato
      ...
      if (value_total != null) {
        this.service
          .getCharacteristic(this._EveTotalConsumption)
          .setValue(value_total, undefined, undefined);
      }
      if (value_voltage1 != null) {
        this.service
          .getCharacteristic(this._EveVoltage1)
          .setValue(value_voltage1, undefined, undefined);
      }
      if (value_ampere1 != null) {
        this.service
          .getCharacteristic(this._EveAmpere1)
          .setValue(value_ampere1, undefined, undefined);
      }
      return true;
    },
    (error) => {
